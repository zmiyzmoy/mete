"""
Unified Product MCP Server - ВЕРСИЯ С AI КЛАССИФИКАЦИЕЙ
Точная реализация всех workflow с автоматической классификацией запросов
"""
...
app = FastAPI(
    ...
    description="Полная реализация всех workflow с AI классификацией",
    version="4.0.0"
)
...
async def classify_query_with_gemini(query: str) -> Dict:
    ...
    try:
        # Используем уже инициализированный Gemini клиент
...
"""
        ...
        # Fallback на поиск товаров при ошибке
        return {
            "type": "PRODUCT_SEARCH",
            "confidence": 0.5,
            "parameters": {}
...
async def process_query(request: UnifiedRequest):
    ...
    try:
        ...
        if classification["type"] == "BUDGET_SPLIT":
            # Создаем ComplexQueryRequest для BUDGET_SPLIT
            complex_request = ComplexQueryRequest(
                query_type="BUDGET_SPLIT",
                tenant_id=request.tenantId,
                sessionId=request.sessionId,
                userPhone=request.userPhone or "",
                params={
                    "quantity": classification["parameters"].get("quantity", 3),
                    "max_budget": classification["parameters"].get("max_budget", 100)
                }
            )
            return await analyze_complex_query(complex_request)
        
        elif classification["type"] == "FIND_EXTREME_PRICE":
            # Создаем ComplexQueryRequest для FIND_EXTREME_PRICE
            complex_request = ComplexQueryRequest(
                ...
                userPhone=request.userPhone or "",
                params={
                    "sort_order": classification["parameters"].get("sort_order", "ASC")
                }
            )
            return await analyze_complex_query(complex_request)
        
        else:  # PRODUCT_SEARCH
            # Создаем SearchProductsRequest
            search_request = SearchProductsRequest(
                query=request.query,
                tenantId=request.tenantId,
                cohereApiKey=request.cohereApiKey,
                sessionId=request.sessionId,
...
async def startup_event():
    ...
    logger.info("Запуск полного MCP сервера с AI классификацией...")
